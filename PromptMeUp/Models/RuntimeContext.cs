...
/// <summary>Describes the minimal runtime facts that make terminal guidance actionable while exposing the machine name only to operational prompts.</summary>
    ...
    string WorkingDirectory)
{
    ...
    /// <summary>Formats approved technical facts, revealing the sanitized working directory and machine name only to an operational prompt.</summary>
    public string ToPromptBlock(string language, bool includeWorkingDirectory)
    {
        ...
        if (includeWorkingDirectory)
        {
            ...
            lines.Add(string.Format(text.MachineName, MachineName));
        }
    ...
    string PrivacyBoundary)
{
    ...
    public static RuntimeContextPromptText For(string? language) => language?.Trim().ToLowerInvariant() switch
    {
        "it" => new(
            ...
            "- Limite di privacy: non sono forniti nome utente, percorsi personali, valori delle variabili d'ambiente o segreti. Non richiederli, dedurli o inventarli."),
        "fr" => new(
            ...
            "- Limite de confidentialité : aucun nom d'utilisateur, chemin personnel, valeur de variable d'environnement ou secret n'est fourni. Ne les demandez pas, ne les déduisez pas et ne les inventez pas."),
        "de" => new(
            ...
            "lokal verfügbar",
            "lokal nicht erkannt",
            "- Datenschutzgrenze: Es werden kein Benutzername, persönlicher Pfad, Umgebungsvariablenwert oder Geheimnis bereitgestellt. Frage nicht danach, leite nichts davon ab und erfinde nichts."),
        "es" => new(
            ...
            "- Límite de privacidad: no se proporciona ningún nombre de usuario, ruta personal, valor de variable de entorno ni secreto. No los solicites, infieras ni inventes."),
        "vi" => new(
            ...
            "- Ranh giới riêng tư: không cung cấp tên người dùng, đường dẫn cá nhân, giá trị biến môi trường hoặc bí mật. Không yêu cầu, suy diễn hoặc bịa ra chúng."),
        _ => new(
            ...
            "- Privacy boundary: no user name, personal path, environment-variable value, or secret is supplied. Do not request, infer, or invent them.")
    };
}

...
public sealed class SqliteDatabaseService : IDatabaseService
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await EnsureDefaultSettingsAsync(connection, cancellationToken).ConfigureAwait(false);
            _logger.LogInformation("SQLite database initialized. SchemaVersion={SchemaVersion}", SchemaVersion);
        }
        finally
        {
            _gate.Release();

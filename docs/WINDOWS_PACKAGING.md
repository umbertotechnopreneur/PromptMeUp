...
For version `0.1.2`, the default output is `artifacts/release/0.1.2/`:

```text
packages/
  PromptMeUp-0.1.2-win-x64.zip
  PromptMeUp-0.1.2-win-arm64.zip
  PromptMeUp-0.1.2-win-x64.msi
  SHA256SUMS.txt
winget/UmbertoGiacobbi.PromptMeUp/0.1.2/
...
```powershell
pwsh -NoProfile -File .\scripts\build-release-artifacts.ps1 `
  -ArtifactBaseUrl 'https://github.com/umbertotechnopreneur/PromptMeUp/releases/download/v0.1.2'
```
...
```powershell
Set-Location .\artifacts\release\0.1.2\packages
...
```powershell
winget install --manifest .\artifacts\release\0.1.2\winget\UmbertoGiacobbi.PromptMeUp\0.1.2 --scope user
```
...
```powershell
msiexec.exe /i .\artifacts\release\0.1.2\packages\PromptMeUp-0.1.2-win-x64.msi
```

For an unattended test in Windows Sandbox or another disposable environment:

```powershell
msiexec.exe /i .\artifacts\release\0.1.2\packages\PromptMeUp-0.1.2-win-x64.msi /qn /norestart
```
...
```powershell
msiexec.exe /x .\artifacts\release\0.1.2\packages\PromptMeUp-0.1.2-win-x64.msi
```
